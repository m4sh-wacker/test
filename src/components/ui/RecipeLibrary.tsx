    ...
    >
      ...
      >
        <input
          ...
          maxLength={80}
          className="min-w-0 flex-1 rounded-control border border-line bg-surface-2 px-2.5 py-1.5 text-xs2 outline-none transition-colors duration-150 focus:border-accent-line"
        />
        <Button
          type="submit"
          variant="primary"
      ...
      <div className="mt-4">
        ...
        {saved.length === 0 ? (
          ...
          <ul className="max-h-64 space-y-1 overflow-y-auto">
            {saved.map((recipe) => (
              <li key={recipe.id}>
                <div className="group flex items-center gap-1 rounded-control border border-line transition-colors duration-150 hover:border-accent-line">
                  <button
                    type="button"
